...
When the backend reports an issue for the blade’s `resourceId`, the pulsing Defender for Cloud RBAC Advisor icon appears in the lower-right corner. Click it to open the recommendation panel, review the alert-informed summary, copy the suggested Azure CLI remediation, or jump to the Defender for Cloud details view.
...
The overlay keeps to a concise Azure-styled layout—badge, summary, and two action buttons—so it feels native when rendered inside portal grids or blades.
